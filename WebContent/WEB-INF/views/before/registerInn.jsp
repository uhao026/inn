...
<body>
<c:if test="${sessionInn.state==1 }">
您的客栈${sessionInn.name }正在审核当中...
</c:if>
<c:if test="${sessionInn.state==2 }">
...
<form action="registerInn" method="post" class="form-horizontal" role="form">
     <input type="hidden" name="id" value="${sessionInn.id }">
     
      <div class="form-group">
		<label for="lastname" class="col-sm-3 control-label">客栈名称</label>
		<div class="col-sm-2">
			<input type="text" class="form-control" id="name" name="name" value="${sessionInn.name }" readonly>
		</div>
		
		<label for="lastname" class="col-sm-2 control-label">地点</label>
		<div class="col-sm-2">
			<input type="text" class="form-control" id="position" name="position" value="${sessionInn.position }" readonly>
		</div>
